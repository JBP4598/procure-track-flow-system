...
((SELECT id FROM departments WHERE code = 'OPS' LIMIT 1), (SELECT id FROM ppmp_files WHERE file_name = 'Operations_PPMP_2024.xlsx' LIMIT 1), (SELECT id FROM profiles WHERE email = 'joshpensawon@gmail.com' LIMIT 1), 150000.00, 'pending', generate_document_number('PR', 'purchase_requests'), 'Safety equipment for operations team');
