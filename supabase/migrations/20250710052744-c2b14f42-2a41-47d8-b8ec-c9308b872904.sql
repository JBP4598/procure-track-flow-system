-- Enable RLS on inspection_reports table (if not already enabled)
ALTER TABLE public.inspection_reports ENABLE ROW LEVEL SECURITY;

-- Enable RLS on iar_items table (if not already enabled)
...
DROP POLICY IF EXISTS "Users can view inspection reports from department POs" ON public.inspection_reports;
...
DROP POLICY IF EXISTS "Users can view IAR items from accessible reports" ON public.iar_items;
